...
public class ItemServiceTest {
    ...
    void get_shouldReturnItemDtoExtendedWithLastAndNextBooking() {
        ...
        Mockito.lenient().when(bookingRepository.findFirst1ByItemIdAndStartIsBeforeAndStatusNot(any(), any(LocalDateTime.class), eq(BookingStatus.REJECTED), any()))
                .thenReturn(lastBooking);

        Mockito.lenient().when(bookingRepository.findFirst1ByItemIdAndStartIsAfterAndStatusNot(any(), any(LocalDateTime.class), eq(BookingStatus.REJECTED), any()))
    ...
    void search_shouldReturnListItemDto() {
        ...
        Mockito.when(itemRepository.findByNameContainingIgnoreCaseOrDescriptionContainingIgnoreCaseAndAvailable(text, text, available))
        ...
        verify(itemRepository).findByNameContainingIgnoreCaseOrDescriptionContainingIgnoreCaseAndAvailable(text, text, available);
    }
    ...
    void addComment_shouldThrowUnavailableItemBookingException() {
        ...
        assertThrows(ObjectNotFoundException.class, () -> itemService.addComment(itemId, userId, comment));
    }
}
